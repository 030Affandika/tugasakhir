<!DOCTYPE html>
<html lang="en">
<body>
<nav class="navbar">
    <img class="logo-instansi" src="../../img/logo_dlhp.png" alt="">
    <div>
        <div>
            <ul class="navbar-container">
    <li class="menu-navbar">
            <a href="dashboard_admin.php">
                <p>Dashboard</p>
                <!-- <img src="../../img/home_fill.png" alt=""> -->
            </a>
        </li>
        <li class="menu-navbar">
            <a href="listProfilPegawai.php">
                <p>Profil Pegawai</p>
                <!-- <img src="../../img/user_fill.png" alt=""> -->
            </a>
        </li>
        <li class="menu-navbar">
            <a href="tmtPengajuan.php">
                <p>TMT Pengajuan</p>
                <!-- <img src="../../img/Desk_alt_fill.png" alt=""> -->
            </a>
        </li>
        <li class="menu-navbar">
            <a href="listProfilPegawaidoc.php">
                <p>Dokumen Pegawai</p>
                <!-- <img src="../../img/Folder_check_fill.png" alt=""> -->
            </a>
        </li>
        <li class="menu-navbar">
            <a href="reminder.php">
                <p>Log Reminder</p>
                <!-- <img src="../../img/Question_fill.png" alt=""> -->
            </a>
        </li>
    </ul>
</div>
<div>
<ul class="logo-navbar-preview">
            <li><img src="../../img/home_fill.png" alt=""></li>
            <li><img src="../../img/user_fill.png" alt=""></li>
            <li><img src="../../img/Desk_alt_fill.png" alt=""></li>
            <li><img src="../../img/Folder_check_fill.png" alt=""></li>
            <li><img src="../../img/File_dock_search_fill.png" alt=""></li>
        </ul>
</div>
    </div>
    <div style="margin-top: 300px;">
        <a class="flex text-white item-center" href="../../../logout.php">
        <p style="margin-left: 50px; font-size:1.2rem;">Logout</p>
        <img style="height: 30px; margin-left: 120px;" src="../../img/logout.png" alt="">    
        </a>
    </div>
    </nav>
    
</body>
</html>